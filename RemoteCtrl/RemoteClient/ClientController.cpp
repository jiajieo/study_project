...
CClientController* CClientController::getInstance()
{
	if (m_instance == NULL) {
		...
		struct { UINT nMsg; MSGFUNC func; }MsgFunc[] = {
...
{
	m_hThread = (HANDLE)_beginthreadex(NULL, 0, &CClientController::threadEntry, this, 0, &m_nThreadID);//CreateThread
	m_statusDlg.Create(IDD_DLG__STATUS, &m_remoteDlg);
	return 0;
}
...
void CClientController::StarWatchScreen()
{
	m_isClosed = false;
...
void CClientController::threadWatchScreen()
{
	...
	while (!m_isClosed) {
		if (m_watchDlg.isFull() == false) {
			...
			if (ret == 6) {
				if (CTroubleTool::Bytes2Image(m_remoteDlg.GetImage(), lstPacks.front().strData) == 0) {
