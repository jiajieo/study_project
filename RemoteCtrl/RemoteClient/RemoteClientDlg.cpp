...
BEGIN_MESSAGE_MAP(CRemoteClientDlg, CDialogEx)
	...
	ON_WM_TIMER()
	ON_NOTIFY(IPN_FIELDCHANGED, IDC_IPADDRESS_SERV, &CRemoteClientDlg::OnlpFieldchangedIpaddressServ)
	ON_EN_CHANGE(IDC_EDIT_PORT, &CRemoteClientDlg::OnEnChangeEditPort)
	ON_MESSAGE(WM_SEND_PACK_ACK, &CRemoteClientDlg::OnSendPacket)
END_MESSAGE_MAP()
...
void CRemoteClientDlg::InitUIData()
{
	...
	UpdateData();
	m_server_address = 0x7F000001;// 0xC0A82B70;//192.168.43.112
	m_nPort = _T("9527");
	CClientController* pController = CClientController::getInstance();
	pController->UpdateAddress(m_server_address, atoi((LPCTSTR)m_nPort));
	UpdateData(FALSE);
	m_dlgStatus.Create(IDD_DLG__STATUS, this);
...
{
	CClientController::getInstance()->StarWatchScreen();
}
...
void CRemoteClientDlg::OnlpFieldchangedIpaddressServ(NMHDR* pNMHDR, LRESULT* pResult)
{
