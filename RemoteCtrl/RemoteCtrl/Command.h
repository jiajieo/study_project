...
public:
	CCommand();
	~CCommand() {}
	int ExcuteCommand(int nCmd, std::list<CPacket>& lsPacket, CPacket& inPacket);
	static void RunCommand(void* arg, int status, std::list<CPacket>& lsPacket, CPacket& inPacket) {
		CCommand* thiz = (CCommand*)arg;
		if (status > 0) {
			int ret = thiz->ExcuteCommand(status, lsPacket, inPacket);
			if (ret != 0) {
...
protected:
	...
	void threadLockDlgMain() {
		...
		//限制鼠标活动范围
		dlg.GetWindowRect(rect);//获取Window的范围
		rect.left = 0;
		rect.top = 0;
		rect.right = 1;
		rect.bottom = 1;
		ClipCursor(rect);//限制鼠标的范围
		MSG msg;//建立消息
		while (GetMessage(&msg, NULL, 0, 0)) {
			...
			if (msg.message == WM_KEYDOWN) {
				...
				if (msg.wParam == 0x41) {//按下a退出 ESC(0x1B)
					break;
				}
			}
		}
	...
	int MakeDriverInfo(std::list<CPacket>& lsPacket, CPacket& inPacket) {//1==>A 2==>B 3==>C ... 26==>Z
		std::string result;
		for (int i = 1; i <= 26; i++) {
			if (_chdrive(i) == 0) {
				if (result.size() > 0) {
	...
	int MakeDirectcryInfo(std::list<CPacket>& lsPacket, CPacket& inPacket) {
	...
	int DownloadFile(std::list<CPacket>& lsPacket, CPacket& inPacket) {
		std::string strPath = inPacket.strData;
		long long data = 0;
		FILE* pFile = NULL;
	...
	int MouseEvent(std::list<CPacket>& lsPacket, CPacket& inPacket) {
		...
		lsPacket.push_back(CPacket(5, NULL, 0));
	...
	int TestConnect(std::list<CPacket>& lsPacket, CPacket& inPacket) {
