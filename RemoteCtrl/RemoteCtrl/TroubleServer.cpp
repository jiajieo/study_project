...
TroubleClient::TroubleClient()
	:m_isbusy(false), m_flags(0),
	m_overlapped(new ACCEPTOVERLAPPED()),
	m_recv(new RECVOVERLAPPED()),
	m_send(new SENDOVERLAPPED()),
	m_vecSend(this, (SENDCALLBACK)&TroubleClient::SendData)
{
...
void TroubleClient::SetOverlapped(PCLIENT& ptr) {
	m_overlapped->m_client = ptr.get();
	m_recv->m_client = ptr.get();
	m_send->m_client = ptr.get();
}
TroubleClient::operator LPOVERLAPPED() {
	return &m_overlapped->m_overlapped;
}

LPWSABUF TroubleClient::RecvWSABuffer()
{
	return &m_recv->m_wsabuffer;
}

LPWSAOVERLAPPED TroubleClient::RecvOverlapped()
{
	return &m_recv->m_overlapped;
}

LPWSABUF TroubleClient::SendWSABuffer()
{
	return &m_send->m_wsabuffer;
}

LPWSAOVERLAPPED TroubleClient::SendOverlapped()
{
	return &m_send->m_overlapped;
}

int TroubleClient::Recv()
{
	int ret = recv(m_sock, m_buffer.data() + m_used, m_buffer.size() - m_used, 0);
	if (ret <= 0)return -1;
	m_used += (size_t)ret;
	CTroubleTool::Dump((BYTE*)m_buffer.data(), ret);
	return 0;
}

int TroubleClient::Send(void* buffer, size_t nSize)
{
...
int TroubleClient::SendData(std::vector<char>& data)
{
	if (m_vecSend.Size() > 0) {
...
bool TroubleServer::StartService() {
	CreateSocket();
...
void TroubleServer::BindNewSocket(SOCKET s)
{
	CreateIoCompletionPort((HANDLE)s, m_hIOCP, (ULONG_PTR)this, 0);
}

void TroubleServer::CreateSocket() {
	WSADATA WSAData;
