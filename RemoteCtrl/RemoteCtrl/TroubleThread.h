...
public:
	...
	bool Start() {
		m_bStatus = true;
		m_hThread = (HANDLE)_beginthread(&TroubleThread::ThreadEntry, 0, this);
		if (!IsValid()) {
			m_bStatus = false;
		}
		return m_bStatus;
...
private:
	...
	static void ThreadEntry(void* arg) {
		TroubleThread* thiz = (TroubleThread*)arg;
		if (thiz) {
			thiz->ThreadWorker();
		}
		_endthread();
...
class TroubleThreadPool {
public:
	TroubleThreadPool(size_t size) {
...
private:
	std::mutex m_lock;
	std::vector<TroubleThread*> m_threads;
};